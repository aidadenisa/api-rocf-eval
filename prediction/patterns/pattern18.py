...
# TODO: TRY TO EXTRACT IT, IT HAS A BIT DIFFERENT THE SKELETONIZE
...
class Pattern18:
  ...
  def get_score(self):
    coords = [852, 229, 1031, 531]
    if self.predictionComplexScores:
      rail_bbox = self.predictionComplexScores['rect'][3]
      ...
      score_rail = self.s.transform(np.array(self.predictionComplexScores['scores'][3]).reshape(-1,1))   
      ...
      if rail_score == 1:
        ...
        if rail_prediction == 1:
          ...
          print('PATTERN18: disegno impreciso')
          label_rail = 1
        else:
          self.drawing = cv2.rectangle(self.drawing, (rail_bbox[0], rail_bbox[1]), (rail_bbox[0]+rail_bbox[2], rail_bbox[1]+rail_bbox[3]), (0,0,255), 2)
          print('PATTERN18: disegno assente')
          label_rail = 0
    else:
      x = coords[0] 
      y = coords[1] 
