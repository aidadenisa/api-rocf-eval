...
class Pattern2:
  ...
  def get_score(self):    
    ...
    rect_ext = np.array([[x[0], x[1]] for x in p_dst])
    ...
    if rect_ext is not None:
        ...
        # the distances between lines that are smaller than 30px
        bigDistances = np.array([l1[1].distance(l2[0]),l2[1].distance(l3[1]),l4[1].distance(l3[0]), l4[0].distance(l1[0])]) > 30
        ...
        linesIntersect = l1[1].intersects(l2[0]) and l2[1].intersects(l3[1]) and l4[1].intersects(l3[0]) and l4[0].intersects(l1[0])
   
        if linesIntersect or ( len(bigDistances[bigDistances == True]) == 0 ):
          # then offer the full score
          label_rect = 3
          rect_ext = np.array([line1_v[0], line1_v[1], line3_v[1], line3_v[0]])
          # build ret from the lines
